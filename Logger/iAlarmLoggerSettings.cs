...
{
    public partial class iAlarmLoggerSettings : UserControl
    {
        ...
        private void IAlarmSettings_Load(object sender, EventArgs e)
        {
            ...
            applicationPath = System.IO.Directory.GetCurrentDirectory();
            var filePathDatabase = applicationPath + "\\DatabaseAlarmLogger.txt";
            FileStream fileStream2;
            if (!System.IO.File.Exists(filePathDatabase))
            {
                fileStream2 = new System.IO.FileStream(filePathDatabase, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None);
            }
            else
            {
                fileStream2 = new System.IO.FileStream(filePathDatabase, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None);
            ...
            try
            {
                using (StreamReader sr1 = new StreamReader(fileStream2))
                {
                    while ((data = sr1.ReadLine()) != null)
                    {
                        var databasepara = data.Split('*');
                        if(databasepara.Length != 5)
                        {
                            break;
                        }
                        this.databaseParametter = new DatabaseParametter
                        {
                            ServerName = databasepara[0],
                            UserID = databasepara[1],
                            Password = databasepara[2],
                            DatabaseName = databasepara[3],
